...
function PlayContent() {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 phase-result">
        <div className="text-center animate-fade-in max-w-sm w-full">
          ...
          <button onClick={() => router.push('/')} className="w-full py-3 bg-doubt-accent rounded-xl font-bold">🏠 خروج</button>
          <p className="text-doubt-muted text-xs mt-2 text-center">أو انتظر المدير يبدأ لعبة جديدة...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`flex flex-col overflow-hidden phase-transition ${phaseBackground}`} style={{ height: 'var(--vh, 100dvh)' }}>
      ...
      {/* Game Bulletin - persistent status bar */}
      {session.isStarted && (killedList.length > 0 || eliminatedList.length > 0) && (
        <div className={`shrink-0 bg-black/60 border-b border-white/5 px-3 py-1.5 flex items-center justify-center gap-4 text-[11px] transition-all duration-300 ${bulletinFlash ? 'bg-doubt-accent/10' : ''}`}>
          {killedList.length > 0 && (
            <span className={`flex items-center gap-1 text-red-400/80 ${bulletinFlash && morningResult?.killed ? 'animate-pulse' : ''}`}>
              💀 قُتل: <span className="text-red-300 font-bold">{killedList.length}</span>
            </span>
          )}
          {eliminatedList.length > 0 && (
            <span className={`flex items-center gap-1 text-amber-400/80 ${bulletinFlash && voteResult?.eliminated ? 'animate-pulse' : ''}`}>
              ⚖️ طُرد: <span className="text-amber-300 font-bold">{eliminatedList.length}</span>
            </span>
          )}
          <span className="flex items-center gap-1 text-green-400/80">
            👥 باقي: <span className="text-green-300 font-bold">{alivePlayers.length}</span>
          </span>
        </div>
      )}
